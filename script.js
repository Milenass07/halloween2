...
function verificarOrientacao() {
    ...
    // Verifica se a tela está no modo retrato (vertical)
    if (window.innerHeight > window.innerWidth) {
        orientationWarning.style.display = 'flex'; // Mostra a mensagem
    } else {
        orientationWarning.style.display = 'none'; // Esconde a mensagem
    }
}

// Adiciona evento de resize para verificar a orientação sempre que a tela for redimensionada
window.addEventListener('resize', verificarOrientacao);

// Chama a função no início para garantir que a orientação seja verificada imediatamente
verificarOrientacao();
